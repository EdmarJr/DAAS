...
public class ConsultarAvisoController extends AvisoController{
	...
	public void init() {
		...
		setAviso((Aviso) FacesContext.getCurrentInstance().getExternalContext().getRequestMap().get("avisoRequest"));
		setListResultadoConsulta((List<Aviso>) FacesContext.getCurrentInstance().getExternalContext().getRequestMap().get("avisoResultadoRequest"));
	...
	public String comandoAlterar() {
		if(avisoSelecionado != null){
			...
			FacesContext.getCurrentInstance().getExternalContext().getRequestMap().put("avisoRequest", getAviso());
			FacesContext.getCurrentInstance().getExternalContext().getRequestMap().put("avisoResultadoRequest", getListResultadoConsulta());
	...
	public String detalharAviso(Aviso avisoSelecionado) {
		...
		FacesContext.getCurrentInstance().getExternalContext().getRequestMap().put("avisoRequest", getAviso());
		FacesContext.getCurrentInstance().getExternalContext().getRequestMap().put("avisoResultadoRequest", getListResultadoConsulta());
		FacesContext.getCurrentInstance().getExternalContext().getRequestMap().put("paginaPai", "/pages/aviso/consultarAviso.xhtml");
